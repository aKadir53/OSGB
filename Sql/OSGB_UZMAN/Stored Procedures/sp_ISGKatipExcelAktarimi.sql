...
begin
  ...
  begin  
    ...
    from ISGKatipExcelAktarim ss
    where GorevTuru = 'Ýçe Grv.'
  	  and LTRIM (RTRIM (IsNull (HizmetAlanKurumSGKSicilNo, ''))) = 'sil'
	  and LTRIM (RTRIM (IsNull (HizmetalanKurum, ''))) = TCKimlikNo
end
  ...
  begin  
    insert into dbo.SIRKETLER_TNM (SirketKod, tanimi, NaceKod, anaFaliyet, tehlikeSinifi, SGKKod, calisanSayisi, Tel1, Tel2, Fax, Yetkili, yetkiliTel, yetkilimail, SEHIR, ILCE, MAHALLE, CADDE, BUCAK, KOY, SOKAK, Adres, VD, VN, Istigal, IsguvenlikUzman, SorunluRevirPersonel, Aktif, User_ID, Date_Create, doktor, IsyeriSicilNo, BolgeMudurlukSicilNo)
