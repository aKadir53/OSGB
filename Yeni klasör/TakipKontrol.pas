...
type
  TfrmTakipKontrol = class(TGirisForm)
    ...
    K1: TMenuItem;
    btnList: TcxButton;
    LayoutHizmetlerGroup_Root: TdxLayoutGroup;
    LayoutHizmetler: TdxLayoutControl;
    ...
    LayoutHizmetlerItem2: TdxLayoutItem;
    ...
    grpTahlil: TcxGroupBox;
    cxGrid2: TcxGrid;
    Tahlil: TcxGridDBTableView;
    ...
    Splitter1: TSplitter;
    TahliltakipNo: TcxGridDBColumn;
    TahlilsutKodu: TcxGridDBColumn;
    TahlilislemTarihi: TcxGridDBColumn;
    TahlilbransKodu: TcxGridDBColumn;
    TahlildrTescilNo: TcxGridDBColumn;
    Tahlilname: TcxGridDBColumn;
    Tahlilsonuc: TcxGridDBColumn;
    TahlilTanim: TcxGridDBColumn;
    Tahlilbirim: TcxGridDBColumn;
    TahlilhizmetSunucuRefNo: TcxGridDBColumn;
    grpRad: TcxGroupBox;
    Splitter2: TSplitter;
    cxGrid5: TcxGrid;
    Rad: TcxGridDBTableView;
    ...
    RadColumn1: TcxGridDBColumn;
    RadColumn2: TcxGridDBColumn;
    ...
    grpMalzeme: TcxGroupBox;
    cxGrid7: TcxGrid;
    cxGridDBTableView3: TcxGridDBTableView;
    cxGridDBTableView3islemTarihi: TcxGridDBColumn;
    cxGridDBTableView3hizmetSunucuRefNo: TcxGridDBColumn;
    cxGridDBTableView3islemSiraNo: TcxGridDBColumn;
    cxGridDBTableView3kodsuzMalzemeAdi: TcxGridDBColumn;
    cxGridDBTableView3adet: TcxGridDBColumn;
    cxGridDBTableView3barkod: TcxGridDBColumn;
    cxGridDBTableView3kodsuzMalzemeFiyati: TcxGridDBColumn;
    cxGridDBTableView3malzemeKodu: TcxGridDBColumn;
    cxGridDBTableView3malzemeTuru: TcxGridDBColumn;
    cxGridDBTableView3paketHaric: TcxGridDBColumn;
    cxGridDBTableView3ozelDurum: TcxGridDBColumn;
...
begin
  Result := False;
  txttarih1.Date := ayliktarih(date,t1);
  txtTarih.Date := t1;
...
procedure TfrmTakipKontrol.btnListClick(Sender: TObject);
var
...
begin
  ...
  //  ADO_TahlilSonuc.Active := false;
    ...
    if ktip.Text = 'Tümü' Then
      ...
      kurumTip := trim(copy(ktip.Text, 1, 2));
...
begin
  ...
  if tag = 1 then
  begin
    i := ADO_Diger.RecordCount;
    SetLength(datalar.islemSiralari, i);

    for j := 0 to ADO_Diger.RecordCount - 1 do
    begin
      datalar.islemSiralari[j] := ADO_Diger.fieldbyname('islemsiraNo').AsString;
      ADO_Diger.Next;
    end;
  ...
  if tag = 2 then
  begin
    i := ADO_Tahlil.RecordCount;
    SetLength(datalar.islemSiralari, i);

    for j := 0 to ADO_Tahlil.RecordCount - 1 do
    begin
      datalar.islemSiralari[j] := ADO_Tahlil.fieldbyname('islemsiraNo')
        .AsString;
      ADO_Tahlil.Next;
    end;
  ...
  begin
    ...
    i := ADO_Malzeme.RecordCount;
    SetLength(datalar.islemSiralari, i);

    for j := 0 to ADO_Malzeme.RecordCount - 1 do
    begin
      datalar.islemSiralari[j] := ADO_Malzeme.fieldbyname('islemsiraNo')
        .AsString;
      ADO_Malzeme.Next;
    end;
  end;
  ...
  if tag = 4 then
  begin
    i := ADO_TetkikveRad.RecordCount;
    SetLength(datalar.islemSiralari, i);

    for j := 0 to ADO_TetkikveRad.RecordCount - 1 do
    begin
      datalar.islemSiralari[j] := ADO_TetkikveRad.fieldbyname('islemsiraNo')
        .AsString;
      ADO_TetkikveRad.Next;
    end;
...
var
  _talepSira, _sonuc, msg, sql: string;
  i, j, k: integer;
  ado: TADOQuery;
begin
  ...
  try
    ...
    begin
      ...
      datalar.islemSiralari[0] := ADO_Diger.fieldbyname('islemSirano').AsString;

      msg := HizmetKaydiIptal(ADO_Diger.fieldbyname('takipno').AsString,
        datalar.HizmetKayit);

      if msg = '0000' Then
      Begin

    //    sql := 'update gelisDetay set ' + ' where TalepSira = ' +
     //     QuotedStr(ADO_Diger.fieldbyname('islemSirano').AsString);
     //   datalar.QueryExec(ado, sql);

        sql := 'update gelisDetay set TalepSira = ''''' +
          ' where TalepSira = ' + QuotedStr(ADO_Diger.fieldbyname('islemSirano').AsString);
        datalar.QueryExec(ado, sql);
        (*
